...
# 店舗DBの「カテゴリ」は12種類（2026-09-26 に「和食・郷土料理」を「和食」「郷土料理」へ分割）。ガイドの見出しは181種類あるので、語で畳む。
...
CATEGORY_RULES = [
    ...
    ("ご当地名物", r"ご当地|名物"),
    # 2026-09-26 ユーザー指示「和食と郷土料理は別の特徴にして」＝旧「和食・郷土料理」を2つに分けた。
    # 郷土料理＝その土地の料理（海外の章名「〜郷土料理」・国内の郷土料理）。和食＝日本料理の業態。
    ("郷土料理", r"郷土"),
    ("和食", r"和食|定食|とんかつ|天ぷら|割烹|料亭|京料理|懐石|会席|日本料理"),
]

# ---- 業態（ピンの絵文字を決める軸・2026-09-26 ユーザー指示） ----
# 「アイコンはレストラン（🍴）、カフェ（☕）、バーor居酒屋orパブ（🍺）、食べ歩き（🍢）、食材の店（🥩）、
#   ベーカリーorパティスリー（🥐）…複数の分類がかぶる場合以下の強弱規則に従って、より強い分類のアイコンが採用される。
#   レストラン＞カフェ＞バーor居酒屋orパブ＞食材の店＞ベーカリーorパティスリー＞食べ歩き」
# 並び＝強い順。店自身の文（店名・一皿・説明）に当たった業態を優先し、無ければ章名・カテゴリ名で当て、
# それでも無ければ「レストラン」（食のガイドなので食事を出す店が既定）。
VENUE_RULES = [
    ("レストラン", r"レストラン|ビストロ|ブラッスリー|ブラスリー|食堂|定食|料亭|割烹|懐石|会席|寿司|鮨|鮓|焼肉|焼き肉|ステーキ|うなぎ|鰻|天ぷら|とんかつ|ラーメン|そば|蕎麦|うどん|中華|イタリア|フレンチ(?!トースト)|フランス料理|洋食|カレー|海鮮|郷土料理|日本料理|和食|ピッツ|ピザ|トラットリア|オステリア|リストランテ|タベルナ|鉄板|鍋|しゃぶ|すき焼|串カツ|料理店|ダイナー|グリル|ガストロ|ミシュラン|ビブグルマン|コース料理|フードホール|フードコート|フードマーケット"),
    ("カフェ", r"(?<!ビア)(?<!ブラウン)(?<!ビール)カフェ|喫茶|珈琲|コーヒー|ティールーム|茶房|茶寮|茶屋|甘味処|ブランチ|カフェテリア"),
    ("バー・居酒屋・パブ", r"(?:^|[\s、。（(「/／&＆]|ワイン|ビア|ビール|カクテル|ウイスキー|ウィスキー|フード|ダイニング|キッチン|スタンディング)バー(?![ムワントフジガベニ])|居酒屋|パブ|酒場|立ち飲み|立呑|ビアカフェ|ビアホール|ブラウンカフェ|ブルーパブ|醸造所|ブルワリー|ワインバー|スナック|(?<!ダル)バール(?!広場)|タップルーム|ビアガーデン|ビアレストラン"),
    ("食材の店", r"市場|マーケット|食材店|チーズ(?!ケーキ)|精肉|肉屋|鮮魚|魚屋|青果|八百屋|惣菜|デリカテッセン|物産|直売|食品店|商店|売店|マスタード|加工肉|漬物|味噌|醤油|豆腐|乾物|茶舗|製茶|茶園|酒店|酒屋"),
    ("ベーカリー・パティスリー", r"ベーカリー|パン屋|パン(?!ケーキ|チ)|パティスリー|ケーキ|洋菓子|和菓子|菓子|スイーツ|チョコレート|ショコラ|プラリネ|ジェラート|アイス|クレープ|ドーナツ|大福|団子|餅|饅頭|カステラ|羊羹|焼き菓子|ビスケット|クッキー|タルト|マカロン|ワッフル"),
    ("食べ歩き", r"屋台|食べ歩き|テイクアウト|フリッツ|フリット|フリテュール|フリトゥール|フリットコット|コロッケ|クロケット|串焼き|たこ焼き|たい焼き|スタンド|キッチンカー|露店|持ち帰り|フードトラック|ホットドッグ|サンドイッチ"),
]
VENUE_ORDER = [v for v, _ in VENUE_RULES]


def venue_of(heading, name="", dish="", desc="", cats=()):
    """店の業態を1つ返す（ピンの絵文字の軸）。VENUE_RULES の並び＝強弱の順。

    ①店自身の文（店名・一皿・説明）に当たった業態の中で最も強いもの
    ②無ければ章名とカテゴリ名に当たった中で最も強いもの
    ③無ければ「レストラン」
    """
    own = " ".join([str(name or ""), str(dish or ""), str(desc or "")])
    head = " ".join([str(heading or "")] + [str(c) for c in (cats or ())])
    for target in (own, head):
        for v, pat in VENUE_RULES:
...
HOME_CUISINE = {
    ...
    # 2026-09-24: フュッセン（バイエルン・アルゴイ）とオーバーヴェーゼル（ライン）の章名を郷土料理に畳む。
    "germany": r"ドイツ料理|バイエルン|アルゴイ|ライン(?:地方|渓谷|の)",
    "croatia": r"クロアチア料理|ダルマチア",
...
def country_of_guide(guide):
    ...
    一度も当たらず、ハワイのご当地料理が「和食（旧・和食・郷土料理）」に落ちる）。
    """
    g = (guide or "").lower()
    ...
    **先頭の区切りまでを先に見る**（その節の主題は先頭にある）。
    ...
    for target in (head, heading):
        for name, pat in CATEGORY_RULES:
            if re.search(pat, target):
                return name
    # どれにも当たらない見出しの受け皿。海外のガイドなら土地の料理、国内なら日本料理と読む（2026-09-26 分割）。
    return "郷土料理" if (country or "").lower() in HOME_CUISINE else "和食"
            ...
            for i in range(1, len(parts), 2):
                ...
                for cid, c in zip(ids, cards):
                    ...
                    rec = {
                        ...
                        "業態": venue_of(heading, c["name"], c["sig"], c["desc"], cats),
                        "選定理由": c["reason"],
