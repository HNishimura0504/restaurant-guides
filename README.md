...
- 北米: `north_america/<国ローマ字>/` に同様（1都市を複数のエリア別ガイドに分ける場合は `<都市>_<エリア>.html` のように都市名を前置する。ホノルルは `waikiki` / `alamoana` / `honolulu_other`）
...
| 兵庫県 | 神戸市 | 72 | [japan/hyogo/](japan/hyogo/) |
| 兵庫県 | 姫路市 | 72 | [japan/hyogo/](japan/hyogo/) |
...
**合計 80冊 / 4251店**（2026-09-26 時点。欧州・北米の店数は各冊の店カードを数え直した値。日本の小計は未検証のまま据え置き）
