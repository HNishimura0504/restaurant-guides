...
- **神奈川（2026-08-20）はHTMLと control ファイルのみ本リポジトリにある**。2026-08-19 制定の `.gitignore`（バイナリは git に入れずドライブへ。理由＝本リポジトリの肥大で、飲食店ガイドの写真がその主因のひとつ）と本ガイドの保存方針が衝突したため、ユーザー判断で**HTML・PDF・写真は別リポジトリへ移す方針**となり、その受け皿（`restaurant-guides`・Private）の作成待ち。**`img/kanagawa配下` が無いため yokohama.html / kawasaki.html は本リポジトリ上では画像が表示されない**。`control/` の research・places・photo_pick・locations・map の各JSONがあれば、調査エージェントを回さずに写真の再取得とHTML・PDFの再生成ができる。既存30冊の移設も同リポジトリ作成後に検討する。
